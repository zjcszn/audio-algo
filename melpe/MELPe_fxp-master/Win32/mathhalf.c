 ...
 *	 IMPLEMENTATION:
 *
 *	   Perform a two's complement dsp_round on the input int32_t with
 *	   saturation.
 *
...
int16_t dsp_round(int32_t L_var1)
{
	int32_t	L_Prod;
	int16_t	var2;

	L_Prod = L_add(L_var1, 0x00008000L);                         /* dsp_round MSP */
 ...
 *	 PURPOSE:
 *
 *	   Shift and dsp_round.  Perform a shift right. After shifting, use
 *	   the last bit shifted out of the LSB to dsp_round the result up
 *	   or down.
 *
 *	 INPUTS:
 *
 ...
 *	 PURPOSE:
 *
 *	   Multiply subtract and dsp_round.  Fractionally multiply two 16
 *	   bit numbers together with saturation.  Subtract from that result
 *	   the 32 bit input with saturation.  Finally dsp_round the result
...
int16_t msu_r(int32_t L_var3, int16_t var1, int16_t var2)
{
	...
	swOut = dsp_round(L_sub(L_var3, L_mult(var1, var2)));
	return (swOut);
}
