...
static void lpfilt(int16_t inbuf[], int16_t lpbuf[], int16_t len)
{
	...
	for (i = 0; i < len; i++){
		...
		lpbuf[PIT_COR_LEN - len + i] = dspround(L_sum);                    /* Q0 */
	}
}
...
static void ivfilt(int16_t ivbuf[], int16_t lpbuf[], int16_t len)
{
	...
	r_coeff[0] = dspround(L_temp);						/* normalized r0 */
	for (i = 1; i < 3; i++){
		L40_sum = 0;
		for (j = i; j < PIT_COR_LEN; j++)
			L40_sum = L40_mac(L40_sum, lpbuf[j], lpbuf[j-i]);
		L_temp = (int32_t) L40_shl(L40_sum, shift);
		r_coeff[i] = dspround(L_temp);
	}
	...
	for (i = 0; i < len; i++){
		...
		ivbuf[PIT_COR_LEN-len+i] = dspround(L_shl(L_temp, 3));
	}
}
